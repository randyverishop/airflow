...
class SpannerInitDBTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        airflow_plugins_dir = os.path.join(airflow_home, "airflow", "plugins")
        if not os.path.exists(airflow_plugins_dir):
            os.makedirs(airflow_plugins_dir)
        if not os.path.isdir(airflow_plugins_dir):
            raise Exception("The plugin dir is not a dir: {}".format(airflow_plugins_dir))
        airflow_spanner_source_plugin_file = os.path.realpath(
            os.path.join(os.path.dirname(__file__),
                         os.pardir,
                         "spanner_airflow_plugin.py"))
        airflow_spanner_target_plugin_file = os.path.join(airflow_plugins_dir,
                                                          "spanner_airflow_plugin.py")
