    ...
    def delete_cluster(
        ...
        try:
            resource = self.get_conn().delete_cluster(
                request = {'project_id': project_id, 'region': self.location, 'cluster_name': name}, retry=retry, timeout=timeout
            )
    ...
    def create_cluster(
        ...
        try:
            resource = self.get_conn().create_cluster(
                request = {'project_id': project_id, 'region': self.location, 'cluster': cluster}, retry=retry, timeout=timeout
            )
            resource = self.wait_for_operation(resource)
            ...
            self.log.info('Assuming Success: %s', error.message)
            return self.get_cluster(request = {'project_id': cluster.name, 'region': project_id})
