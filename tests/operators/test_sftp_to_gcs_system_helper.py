...
from airflow.example_dags.example_sftp_to_gcs import BUCKET_SRC, OBJECT_SRC_1, OBJECT_SRC_2
...
class SFTPtoGcsTestHelper(LoggingCommandExecutor):
    ...
    files_and_dirs = [
        ("tezt_file_1.bin", os.path.join(TMP_PATH, TMP_DIR_FOR_TESTS)),
        ("tezt_file_2.bin", os.path.join(TMP_PATH, TMP_DIR_FOR_TESTS)),
        ("tezt_file_3.bin", os.path.join(TMP_PATH, TMP_DIR_FOR_TESTS)),
    ]
    ...
    @staticmethod
    def _delete_temp_file(filename, dir_path="/tmp"):
        full_path = os.path.join(dir_path, filename)
        try:
            os.remove(full_path)
        except FileNotFoundError:
            pass
        if dir_path is not "/tmp":
