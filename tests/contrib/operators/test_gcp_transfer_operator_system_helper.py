...
class GCPTransferTestHelper(LoggingCommandExecutor):
    ...
    def create_s3_bucket(self):
        self.execute_cmd([
            "aws", "s3", "mb", "s3://%s" % GCP_TRANSFER_SOURCE_AWS_BUCKET
        ])
        ...
        self.execute_cmd([
            ...
            (TEST_FILE_NO, GCP_TRANSFER_SOURCE_AWS_BUCKET,
             GCP_TRANSFER_SOURCE_AWS_BUCKET)
        ])

    def delete_s3_bucket(self):
        self.execute_cmd([
            "aws", "s3", "rb", "s3://%s" % GCP_TRANSFER_SOURCE_AWS_BUCKET, "--force"
        ])
    ...
    def create_gcs_buckets(self):
        ...
        self.execute_cmd([
            'gsutil', 'mb', "-p", GCP_PROJECT_ID, "-c", "regional", "-l", "asia-east1",
    ...
    def delete_gcs_buckets(self):
        self.execute_cmd([
            'gsutil', 'rm', "-r", "gs://%s/" % GCP_TRANSFER_FIRST_TARGET_BUCKET, "&"
        ], True)

        self.execute_cmd([
            'gsutil', 'rm', "-r", "gs://%s/" % GCP_TRANSFER_SECOND_TARGET_BUCKET, "&"
        ], True)
