...
class BigQueryHook(GoogleBaseHook, DbApiHook):
    ...
    def monkey_patch_connection_form(form_to_patch):
        ...
        form_to_patch.extra__google_cloud_platform__project = StringField(
            lazy_gettext('Project Id'), widget=BS3TextFieldWidget()
        )
        form_to_patch.extra__google_cloud_platform__key_path = StringField(
            lazy_gettext('Keyfile Path'), widget=BS3TextFieldWidget()
        )
        form_to_patch.extra__google_cloud_platform__keyfile_dict = PasswordField(
            lazy_gettext('Keyfile JSON'), widget=BS3PasswordFieldWidget()
        )
        form_to_patch.extra__google_cloud_platform__scope = StringField(
            lazy_gettext('Scopes (comma separated)'), widget=BS3TextFieldWidget()
        )
        form_to_patch.extra__google_cloud_platform__num_retries = IntegerField(
