...
class GrpcHook(BaseHook):
    ...
    def monkey_patch_connection_form():
        ...
        ConnectionForm.extra__grpc__auth_type = StringField(
            lazy_gettext('Grpc Auth Type'), widget=BS3TextFieldWidget()
        )
        ConnectionForm.extra__grpc__credential_pem_file = StringField(
            lazy_gettext('Credential Keyfile Path'), widget=BS3TextFieldWidget()
        )
        ConnectionForm.extra__grpc__scopes = StringField(
