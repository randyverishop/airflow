...
from flask import current_app as flask_app, g
...
class AirflowSecurityManager(SecurityManager, LoggingMixin):
    ...
    def get_user_roles(user=None):
        ...
        if user.is_anonymous:
            public_role = flask_app.appbuilder.config.get('AUTH_ROLE_PUBLIC')
            return [flask_app.appbuilder.security_manager.find_role(public_role)] \
                if public_role else []
        return user.roles
